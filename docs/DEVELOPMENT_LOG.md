...
### Waseda Primitive Event Pipeline Updated For Event Classes

- Updated [analyzePrimitiveEvents.m](/Users/yoe/Documents/REPOS/eMove-playground/CODE/ACCELEROMETER/analyzePrimitiveEvents.m)
  so the main folder-level Waseda event pipeline now carries the stabilized
  unitary/compound definitions into subject and condition summaries.
- Definition preserved:
  - primary events are still peaks in
    `eventSignal = max(motionEnvelope - localBaseline, 0)`
  - unitary/compound class is inherited from the current
    valley-delimited same-bout fields generated by `extractEnvelopeEvents`
  - `hasNearbyPeak` / `isNearbyPeakEvent` remain older proximity/context
    flags and are not the class definition
- New output fields include:
  - `eventClassSummaryTable`
  - `conditionEventClassSummaryTable`
  - `subjectEventClassSummaryTable`
  - `boutTable`
  - `boutConditionEventClassSummaryTable`
  - `eventClassMeanWaveformTable`
- Average waveform rule:
  - event-class waveform comparisons are now onset-aligned, not peak-aligned
  - unitary bouts align to the low-threshold onset before their only subpeak
  - compound bouts align to the low-threshold onset before the first
    same-bout subpeak
  - the onset threshold is `2 * median(noiseSigma)` by default, matching the
    lower same-bout subpeak threshold
  - peak-aligned mean-waveform outputs are no longer generated by
    `analyzePrimitiveEvents`
  - the event-class waveform figure now includes both raw-amplitude means and
    per-event peak-amplitude-normalized means
- Event-class CDF figures:
  - condition-by-class CDF panels are generated for amplitude, interval, and
    detector width
  - subject-by-class CDF panels are generated for the same metrics
- Reproduction/promotion:
  - [run_waseda_event_class_pipeline.m](/Users/yoe/Documents/REPOS/eMove-playground/scripts/run_waseda_event_class_pipeline.m)
    regenerates the scratch outputs and promotes the curated `.png/.fig`
    comparison figures
  - promoted figure folder:
    `figures/WASEDA_ACC_EVENT_CLASSES_20260508/`
- Validation run:
  - source magnitude folder:
    `/Users/yoe/Dropbox/WORK/Data/Waseda-ACC/MAGNITUDES`
  - output folder:
    `scratch/waseda_event_class_pipeline_20260508_onset_only/`
  - scalar primary-event counts:
    `205` unitary anchors and `534` compound anchors
  - onset-aligned waveform snippets:
    `204` unitary bouts and `534` compound bouts after boundary-window
    exclusions
  - grant-style onset-aligned reproduction remains:
    `735` usable bouts, `203` unitary and `532` compound, because that
    figure applies its own fixed output-window exclusions
