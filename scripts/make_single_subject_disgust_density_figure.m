% make_single_subject_disgust_density_figure.m
%
% Build a single-subject disgust-focused density figure for head and torso.
...
outDir = fullfile(repoRoot, 'outputs', 'figures', ['single_subject_disgust_density_' runStamp]);
if ~exist(outDir, 'dir')
    mkdir(outDir);
end
...
for normIdx = 1:2
    ...
    for r = 1:numel(markerGroups)
        ...
        for e = 1:numel(emotionList)
            emo = emotionList{e};
            fullVals{e} = localApplyOutlierCut(localCollectRawSamplesForSubjectNormalized( ...
                rc, vidToEmotion, mg, emo, 'speedArray', doBaselineNormalize, ...
                baselineEmotion, 'speedArray', minBaselineSamples), outlierQuantile);
            microVals{e} = localApplyOutlierCut(localCollectRawSamplesForSubjectNormalized( ...
                rc, vidToEmotion, mg, emo, 'speedArrayImmobile', doBaselineNormalize, ...
                baselineEmotion, 'speedArrayImmobile', minBaselineSamples), outlierQuantile);

            summaryRows(end+1, :) = {char(subjectID), normLabel, mg, emo, ...
    ...
    exportgraphics(f, fullfile(outDir, sprintf('single_subject_disgust_density_%s.png', strrep(normLabel, '-', '_'))), 'Resolution', 220);
    exportgraphics(f, fullfile(outDir, sprintf('single_subject_disgust_density_%s.pdf', strrep(normLabel, '-', '_'))), 'ContentType', 'vector');
    savefig(f, fullfile(outDir, sprintf('single_subject_disgust_density_%s.fig', strrep(normLabel, '-', '_'))));
end

summaryTbl = cell2table(summaryRows, ...
    'VariableNames', {'subjectID','normalization','markerGroup','emotion','nFull','medianFull','nMicro','medianMicro'});
writetable(summaryTbl, fullfile(outDir, 'single_subject_disgust_density_summary.csv'));

fprintf('Saved single-subject disgust density figures under:\n%s\n', outDir);
