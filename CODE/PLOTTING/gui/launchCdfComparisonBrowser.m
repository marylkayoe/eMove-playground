function H = launchCdfComparisonBrowser(varargin)
% launchCdfComparisonBrowser - Interactive browser for group-level CDF comparisons.
%
% Usage:
%   launchCdfComparisonBrowser()
%   launchCdfComparisonBrowser('resultsCellPath', '/path/to/resultsCell.mat')
%
% This GUI lets the user choose:
%   - one or more bodypart marker groups
%   - one or more emotions to compare
%   - full-speed vs micromovement regime
%   - median vs raw plotting mode
...
%     using painters rendering (`print -depsc`).
    ...
    addParameter(p, 'defaultPlotMode', 'perVideoMedian', @(x) ischar(x) || isstring(x));
    ...
    addParameter(p, 'defaultMinSamplesPerSubj', 200, @(x) isnumeric(x) && isscalar(x) && x >= 0);
    parse(p, varargin{:});
    ...
    if ~isfile(resultsCellPath)
        error('launchCdfComparisonBrowser:BadResultsCell', 'resultsCell file not found: %s', resultsCellPath);
    end
    if ~isfile(stimCsv)
        error('launchCdfComparisonBrowser:BadStimCsv', 'Stim CSV not found: %s', stimCsv);
    end
    ...
    if ~isfield(Sload, 'resultsCell')
        error('launchCdfComparisonBrowser:MissingResultsCell', 'resultsCell variable missing in %s', resultsCellPath);
    end
    ...
    f = figure( ...
        'Name', 'CDF Comparison Browser', ...
        'NumberTitle', 'off', ...
        'Color', 'w', ...
        'MenuBar', 'none', ...
        'ToolBar', 'none', ...
        'Units', 'pixels', ...
        'Position', [120 90 720 800]);
    ...
    uicontrol(f, 'Style', 'text', 'String', 'Plot mode', ...
        'HorizontalAlignment', 'left', 'BackgroundColor', 'w', ...
        'Position', [rightLabelX 704 120 20]);
    hPlotMode = uicontrol(f, 'Style', 'popupmenu', ...
        'String', {'perVideoMedian','pooledRaw','perSubjectRaw'}, ...
        'Value', localFindStringIndex({'perVideoMedian','pooledRaw','perSubjectRaw'}, ...
            char(string(p.Results.defaultPlotMode)), 1), ...
        'Position', [rightFieldX 700 rightFieldW 28], ...
    ...
    hImmobile = uicontrol(f, 'Style', 'checkbox', ...
        ...
        'BackgroundColor', 'w', ...
        'Position', [rightLabelX 654 180 24], ...
        'Callback', @onAutoRefresh);
    ...
    hNormalize = uicontrol(f, 'Style', 'checkbox', ...
        ...
        'BackgroundColor', 'w', ...
        'Position', [rightLabelX 622 180 24], ...
        'Callback', @onAutoRefresh);
    ...
    hStats = uicontrol(f, 'Style', 'checkbox', ...
        ...
        'Value', 1, ...
        'BackgroundColor', 'w', ...
        'Position', [rightLabelX 590 140 24], ...
        'Callback', @onAutoRefresh);

    uicontrol(f, 'Style', 'text', 'String', 'Min samples / subj', ...
        'HorizontalAlignment', 'left', 'BackgroundColor', 'w', ...
        'Position', [rightLabelX 548 130 20]);
    hMinSamples = uicontrol(f, 'Style', 'edit', ...
        'String', num2str(p.Results.defaultMinSamplesPerSubj), ...
        'BackgroundColor', 'w', ...
        'Position', [rightFieldX 544 rightFieldW 28], ...
        'Callback', @onNumericEdited);

    uicontrol(f, 'Style', 'text', 'String', 'Immobility threshold', ...
        'HorizontalAlignment', 'left', 'BackgroundColor', 'w', ...
        'Position', [rightLabelX 506 130 20]);
    hImmThr = uicontrol(f, 'Style', 'edit', ...
        'String', num2str(p.Results.defaultImmobilityThreshold), ...
        'BackgroundColor', 'w', ...
        'Position', [rightFieldX 502 rightFieldW 28], ...
        'Callback', @onNumericEdited);
    ...
    function onRefresh(~, ~)
        ...
        try
            ...
            if numel(settings.emotions) < 2
                error('Choose at least two emotions.');
            end
            ...
            figBefore = figBefore(figBefore ~= f);

            if settings.useImmobile
                summaryField = 'medianSpeedImmobile';
                immobilityField = 'speedArrayImmobile';
                regimeLabel = sprintf('micromovement (<= %g mm/s)', settings.immobilityThreshold);
            else
            ...
            if strcmp(settings.plotMode, 'perVideoMedian')
            ...
            figTitle = sprintf('%s | %s | %s | %s', ...
                strjoin(settings.emotions, ' vs '), ...
                strjoin(localPrettyMarkerGroupLabels(settings.markerGroups), ', '), ...
                localPlotModeLabel(settings.plotMode), ...
                regimeLabel);
            ...
            plotArgs = { ...
                ...
                'immobilityField', immobilityField, ...
                'summaryField', summaryField, ...
                'doBaselineNormalize', settings.doBaselineNormalize, ...
                'baselineEmotion', 'BASELINE', ...
                'baselineFromField', baselineFromField, ...
                'minBaselineSamples', 20, ...
                'minSamplesPerSubj', settings.minSamplesPerSubj, ...
                'showStats', settings.showStats, ...
                'tileCols', min(2, numel(settings.markerGroups)), ...
                'figureTitle', figTitle};
            if numel(settings.emotions) == 2
                plotArgs = [plotArgs, {'statsPair', settings.emotions}];
            end

            plotSpeedCDFByStimGroupFromResultsCell(S.state.resultsCell, S.state.codingTable, ...
                plotArgs{:});

            figAfter = findall(groot, 'Type', 'figure');
            newFigs = setdiff(figAfter, [figBefore; f]);
            if isempty(newFigs)
                error('No plot figure was created.');
            end
            plotFig = newFigs(1);
            set(plotFig, 'Name', sprintf('CDF Comparison - %s', strjoin(settings.emotions, ' vs ')));
            S.state.currentPlotFigure = plotFig;
            set(S.hStatus, 'String', sprintf('Plotted %s | %s', ...
                strjoin(settings.emotions, ', '), regimeLabel), ...
                'ForegroundColor', [0.2 0.2 0.2]);
            guidata(f, S);
        catch ME
...
function settings = localReadControls(S)
    modeItems = get(S.hPlotMode, 'String');

    settings = struct();
    settings.emotions = {};
    settings.plotMode = char(string(modeItems{get(S.hPlotMode, 'Value')}));
    ...
    settings.minSamplesPerSubj = max(0, round(localParseNumericEdit(S.hMinSamples, 200)));
...
function label = localPlotModeLabel(plotMode)
...
function idx = localFindStringIndex(items, targetValue, fallbackIdx)
    ...
    if isempty(items) || isempty(targetValue)
        return;
    end
    itemsCell = cellstr(string(items));
    matchIdx = find(strcmp(itemsCell, char(string(targetValue))), 1, 'first');
    if ~isempty(matchIdx)
        idx = matchIdx;
    end
end
