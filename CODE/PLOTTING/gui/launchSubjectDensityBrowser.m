...
%   - the micromovement regime is currently taken from the precomputed
%     `speedArrayImmobile` field already stored in `resultsCell`
%     (currently built with the analysis-side immobility threshold),
%     so this browser does not yet support live threshold changes
...
subjectIDs = [{'POOLED (all subjects)'}; localCollectSubjectIDs(resultsCell)];
...
state.fixedImmobilityThreshold = p.Results.defaultImmobilityThreshold;
...
hShowKs = uicontrol(f, 'Style', 'checkbox', ...
    'String', 'Show KS (2 emotions)', ...
    'Value', 1, ...
    'BackgroundColor', 'w', ...
    'Position', [rightLabelX 608 180 24], ...
    'Callback', @onControlsChanged);
...
hPlotMode = uicontrol(f, 'Style', 'popupmenu', ...
    'String', {'Probability density','CDF'}, ...
    'Value', 1, ...
...
uicontrol(f, 'Style', 'text', 'String', 'X-limit quantile', ...
    'HorizontalAlignment', 'left', 'BackgroundColor', 'w', ...
    'Position', [rightLabelX 560 140 20]);
hXLimitQuantile = uicontrol(f, 'Style', 'edit', ...
    'String', num2str(p.Results.defaultXLimitQuantile), ...
    'BackgroundColor', 'w', ...
    'Position', [rightFieldX 556 rightFieldW 28], ...
    'Callback', @onNumericEdited);

uicontrol(f, 'Style', 'text', ...
    'String', sprintf('Micromovement uses precomputed threshold (currently %g mm/s)', p.Results.defaultImmobilityThreshold), ...
    'HorizontalAlignment', 'left', ...
    'BackgroundColor', 'w', ...
    'ForegroundColor', [0.35 0.35 0.35], ...
    'Position', [rightLabelX 516 300 36]);
...
hHeatmap = uicontrol(f, 'Style', 'pushbutton', ...
    'String', 'Plot KS Heatmap', ...
    'Position', [380 244 170 44], ...
    'Callback', @onPlotHeatmap);
hExportEps = uicontrol(f, 'Style', 'pushbutton', ...
    'String', 'Export EPS', ...
    'Position', [560 244 170 44], ...
...
if nargout > 0
    ...
    function onRefresh(~, ~)
        localPlotCurrentSelection('density');
    end

    function onPlotHeatmap(~, ~)
        localPlotCurrentSelection('heatmap');
    end

    function localPlotCurrentSelection(plotKind)
        S = guidata(f);
        try
            ...
            minEmotionCount = ternary(strcmp(plotKind, 'heatmap'), 2, 1);
            if numel(settings.emotions) < minEmotionCount
                if strcmp(plotKind, 'heatmap')
                    error('Choose at least two emotions for the heatmap.');
                else
            ...
            isPooled = strcmp(subjectID, 'POOLED (all subjects)');
            regimeLabel = ternary(settings.useImmobile, ...
                sprintf('micromovement (precomputed <= %g mm/s)', settings.immobilityThreshold), ...
                'full motion');

            if strcmp(plotKind, 'heatmap')
                figTitle = sprintf('%s | pairwise KS heatmap | %s', subjectID, regimeLabel);
                if isPooled
                    plotFig = plotPooledPairwiseStatsHeatmap(S.state.resultsCell, S.state.codingTable, ...
                        ...
                        'outlierQuantile', settings.xLimitQuantile, ...
                        'figureTitle', figTitle);
                    figName = 'Pooled KS Heatmap';
                else
                    rcIdx = S.state.subjectIndexMap(subjectID);
                    rc = S.state.resultsCell{rcIdx};
                    plotFig = plotSubjectPairwiseStatsHeatmap(rc, S.state.codingTable, ...
                        'markerGroups', settings.markerGroups, ...
                        'markerGroupAliases', S.state.markerGroupAliasMap, ...
                        'emotions', settings.emotions, ...
                        'useImmobile', settings.useImmobile, ...
                        ...
                        'outlierQuantile', settings.xLimitQuantile, ...
                        'figureTitle', figTitle);
                    figName = sprintf('Subject KS Heatmap - %s', subjectID);
                end
                statusVerb = 'Heatmap';
            else
                ...
                if isPooled
                    plotFig = plotPooledSpeedDensityByEmotion(S.state.resultsCell, S.state.codingTable, ...
                        'markerGroups', settings.markerGroups, ...
                        'markerGroupAliases', S.state.markerGroupAliasMap, ...
                        'emotions', settings.emotions, ...
                        'useImmobile', settings.useImmobile, ...
                        ...
                        'showKs', settings.showKs);
                    figName = 'Pooled Density';
                else
                    rcIdx = S.state.subjectIndexMap(subjectID);
                    rc = S.state.resultsCell{rcIdx};
                    plotFig = plotSubjectSpeedDensityByEmotion(rc, S.state.codingTable, ...
                        ...
                        'showStats', settings.showStats, ...
                        'showKs', settings.showKs);
                    figName = sprintf('Subject Density - %s', subjectID);
                end
                statusVerb = 'Plotted';
            end

            set(plotFig, 'Name', figName);
            set(plotFig, 'Tag', 'SubjectDensityBrowserPlot');
            S.state.currentPlotFigure = plotFig;
            set(S.hStatus, 'String', sprintf('%s %s | %s', statusVerb, subjectID, regimeLabel), ...
                'ForegroundColor', [0.2 0.2 0.2]);
            guidata(f, S);
        catch ME
...
settings.showKs = logical(get(S.hShowKs, 'Value'));
plotModes = get(S.hPlotMode, 'String');
settings.plotMode = char(string(plotModes{get(S.hPlotMode, 'Value')}));
settings.xLimitQuantile = min(1, max(0.5, localParseNumericEdit(S.hXLimitQuantile, 0.95)));
settings.immobilityThreshold = S.state.fixedImmobilityThreshold;
settings.emotions = {};
for i = 1:numel(S.state.emotionCheckboxes)
