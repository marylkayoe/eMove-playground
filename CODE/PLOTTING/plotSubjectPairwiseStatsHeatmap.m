function fig = plotSubjectPairwiseStatsHeatmap(rc, codingTable, varargin)
% plotSubjectPairwiseStatsHeatmap
% Plot compact per-subject pairwise emotion heatmaps for selected bodyparts.
%
% Usage:
%   fig = plotSubjectPairwiseStatsHeatmap(rc, codingTable, ...
%       'markerGroups', {'HEAD','UTORSO','LEGS'}, ...
%       'emotions', {'NEUTRAL','DISGUST','FEAR','JOY','SAD'}, ...
%       'useImmobile', true, ...
%       'doBaselineNormalize', true)
%
% Display convention:
%   Off-diagonal cells:
%       cell color = KS distance D from kstest2
%       text       = ranksum significance stars only
%   Diagonal:
%       blank separator
%
% Notes:
%   - The KS color scale is shared across all selected bodypart panels.
%   - This is intended as a compact exploratory view from the density browser.
...
if isempty(markerGroups)
    error('plotSubjectPairwiseStatsHeatmap:NoMarkerGroups', 'At least one marker group is required.');
end
if numel(emotionList) < 2
    error('plotSubjectPairwiseStatsHeatmap:TooFewEmotions', 'At least two emotions are required.');
end
if ~isfield(rc, 'summaryTable') || isempty(rc.summaryTable)
    error('plotSubjectPairwiseStatsHeatmap:MissingSummary', 'Subject entry has no summaryTable.');
end
...
if isempty(strtrim(figureTitle))
    ...
    figureTitle = sprintf('%s | pairwise stats | %s | %s', subjectID, normLabel, regimeLabel);
end
...
title(tl, figureTitle, 'FontSize', 24, 'FontWeight', 'bold', 'Interpreter', 'none');
...
for g = 1:nGroups
    ...
    for e = 1:numel(emotionList)
        vals = localApplyOutlierCut(localCollectRawSamplesForSubjectNormalized( ...
    ...
    for i = 1:nE
        ...
        valsI = valsI(:);
        valsI = valsI(~isnan(valsI) & isfinite(valsI));
        for j = i+1:nE
            valsJ = sampleMap(emotionList{j});
            valsJ = valsJ(:);
            valsJ = valsJ(~isnan(valsJ) & isfinite(valsJ));
...
for g = 1:nGroups
    ...
    axis(ax, 'image');
    set(ax, 'YDir', 'normal', 'FontSize', 14, 'LineWidth', 1.0, 'Box', 'off');
    ...
    xticks(ax, 1:nE);
    yticks(ax, 1:nE);
    xticklabels(ax, emotionList);
    yticklabels(ax, emotionList);
    xtickangle(ax, 40);
    grid(ax, 'on');
    ax.GridColor = [1 1 1];
    ax.GridAlpha = 0.25;
    title(ax, localPrettyMarkerGroupLabel(mg), 'FontSize', 18, 'FontWeight', 'bold');

    for i = 1:nE
        rectangle(ax, 'Position', [i-0.5, i-0.5, 1, 1], ...
        ...
        for j = 1:nE
            ...
            text(ax, j, i, txt, ...
                ...
                'FontSize', 15, ...
                'FontWeight', 'bold', ...
                'Color', localTextColorForValue(D(i,j), colorHi));
        end
    end
end

caption = 'All off-diagonal cells show KS distance D by color. Cell text shows ranksum significance only. Color scale is shared across bodyparts in this figure.';
